...
<div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div class="w-full sm:w-auto">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold">{{ $category->name }}</h1>
    ...
    @auth
        <a href="{{ route('forum.thread.create', $category->slug) }}" 
           class="w-full sm:w-auto px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-white font-medium transition-colors text-center">
            New Thread
        </a>
    @endauth
</div>
        ...
        @forelse($threads as $thread)
            <div class="px-4 sm:px-6 py-4 hover:bg-gray-800 transition-colors">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0">
                    ...
                    <div class="flex sm:hidden gap-4 text-sm">
                        <div class="text-gray-300">
                            <span class="text-gray-400">Replies:</span> {{ $thread->posts_count ?? 0 }}
                        </div>
                        <div class="text-gray-300">
                            <span class="text-gray-400">Likes:</span> {{ $thread->likes_count ?? 0 }}
                        </div>
                        <div class="text-gray-300">
                            <span class="text-gray-400">Views:</span> {{ number_format($thread->views) }}
                        </div>
                    </div>
                    <div class="hidden sm:flex sm:w-20 md:w-24 text-center text-gray-300 justify-center">
